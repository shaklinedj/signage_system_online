...
// Recuperar la lista de casinos desde la base de datos
$casinos = [];
$sql_casinos = "SELECT id, casino FROM casino";
$result_casinos = mysqli_query($link, $sql_casinos);
while ($row = mysqli_fetch_assoc($result_casinos)) {
    $casinos[] = $row;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($usuario_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        $sql = "INSERT INTO users (username, password, casino_id) VALUES (?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssi", $param_usuario, $param_password, $casino_id);
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                <label>Casino</label>
                <select name="casino_id" class="form-control">
                <?php
                    foreach ($casinos as $casino) {
                        echo "<option value='" . $casino['id'] . "'>" . $casino['casino'] . "</option>";
                    }
                    ?>
                </select>
            </div>
